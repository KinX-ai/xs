  ...
  async saveLotteryResults(results: any): Promise<LotteryResult> {
    try {
      const lotteryResultToInsert = {
        date: results.date instanceof Date ? results.date : new Date(results.date),
        results: results.results,
        createdAt: new Date()
      };

      console.log("Inserting lottery result:", lotteryResultToInsert);
      ...
      console.log("Successfully inserted result:", result[0]);
      return result[0];
    } catch (error) {
