...
function validateBetNumbers(numbers: string[], betType: string) {
  ...
  app.get("/api/lottery/results/latest", async (req, res) => {
    try {
      ...
      // Get all results
      const allResults = await storage.getLotteryResults();
      
      // If no results exist, return mock data
      if (!allResults || allResults.length === 0) {
        return res.json(createMockLotteryResult());
      }

      // If time is before 18:30, use yesterday's date
      ...
      if (hours < 18 || (hours === 18 && minutes < 30)) {
        // Before 18:30 - show result with adjusted date
        return res.json({
          ...allResults[0],
          date: formattedDate,
          drawState: "complete"
        });
      } else if ((hours === 18 && minutes >= 15) || (hours === 19 && minutes < 15)) {
        // During drawing time (18:15 - 19:15)
      ...
      try {
        ...
        if (apiData && apiData.data && apiData.data.length > 0) {
          ...
          const historicalResults = apiData.data.map(data => {
            const drawDate = new Date(data.openTime);
            ...
            return {
              ...
              results: {
                special: processNumber(data.dacbiet) || "",
                first: processNumber(data.nhat) || "",
                second: processArrayResults(data.nhi ? data.nhi.split(",") : []),
                third: processArrayResults(data.ba ? data.ba.split(",") : []),
                fourth: processArrayResults(data.tu ? data.tu.split(",") : []),
                fifth: processArrayResults(data.nam ? data.nam.split(",") : []),
                sixth: processArrayResults(data.sau ? data.sau.split(",") : []),
                seventh: processArrayResults(data.bay ? data.bay.split(",") : [])
              }
            };
          });

          // Save all historical results to database
          ...
          for (const result of historicalResults) {
            try {
              ...
              const exists = existingResults.some(existing => {
                ...
                return existingDate === result.date;
              });
              
              if (!exists) {
                console.log("Saving new result for date:", result.date);
                const lotteryResult = {
                  date: new Date(result.date),
                  results: result.results
                };
                await storage.saveLotteryResults(lotteryResult);
                console.log("Successfully saved results for date:", result.date);
              } else {
                console.log("Results already exist for date:", result.date);
              }
            } catch (error) {
              console.error("Error saving results for date:", result.date, error);
            }
          }
          ...
          const processedResults = {
            ...
            drawTime: "18:15 - 19:15",
            results: {
              special: processNumber(special) || (isDrawing ? "" : "92568"),
              first: processNumber(first) || (isDrawing ? "" : "48695"),
              second: processArrayResults(second) || (isDrawing ? Array(2).fill("") : ["92735", "19304"]),
              third: processArrayResults(third) || (isDrawing ? Array(6).fill("") : ["39857", "90815", "16359", "83649", "21947", "12376"]),
              fourth: processArrayResults(fourth) || (isDrawing ? Array(4).fill("") : ["1947", "3658", "7539", "5824"]),
              fifth: processArrayResults(fifth) || (isDrawing ? Array(6).fill("") : ["5297", "8714", "3852", "2957", "0463", "3175"]),
              sixth: processArrayResults(sixth) || (isDrawing ? Array(3).fill("") : ["794", "359", "651"]),
              seventh: processArrayResults(seventh) || (isDrawing ? Array(4).fill("") : ["58", "94", "71", "23"])
            }
          };

          console.log("Successfully fetched results from xoso188.net API");
          await storage.saveLotteryResults(processedResults);
    ...
    if (resultTable.length > 0) {
      ...
      if (specialPrize || firstPrize || secondPrizes.length > 0) {
        ...
        const processedResults = {
          ...
          drawTime: "18:15 - 19:15",
          results: {
            special: processNumber(specialPrize) || "92568",
            first: processNumber(firstPrize) || "48695",
            second: processArrayResults(secondPrizes) || ["92735", "19304"],
            third: processArrayResults(thirdPrizes) || ["39857", "90815", "16359", "83649", "21947", "12376"],
            fourth: processArrayResults(fourthPrizes) || ["1947", "3658", "7539", "5824"],
            fifth: processArrayResults(fifthPrizes) || ["5297", "8714", "3852", "2957", "0463", "3175"],
            sixth: processArrayResults(sixthPrizes) || ["794", "359", "651"],
            seventh: processArrayResults(seventhPrizes) || ["58", "94", "71", "23"]
          }
        };
    ...
    if (scriptContent && scriptContent[1]) {
      try {
        ...
        const processedResults = {
          ...
          drawTime: "18:15 - 19:15",
          results: {
            special: processNumber(prizeData.db) || "92568",
            first: processNumber(prizeData.nhat) || "48695",
            second: processArrayResults(prizeData.nhi ? [prizeData.nhi] : []) || ["92735", "19304"],
            third: processArrayResults((prizeData.ba || "").split(" ").filter(Boolean)) || ["39857", "90815", "16359", "83649", "21947", "12376"],
            fourth: processArrayResults((prizeData.tu || "").split(" ").filter(Boolean)) || ["1947", "3658", "7539", "5824"],
            fifth: processArrayResults((prizeData.nam || "").split(" ").filter(Boolean)) || ["5297", "8714", "3852", "2957", "0463", "3175"],
            sixth: processArrayResults((prizeData.sau || "").split(" ").filter(Boolean)) || ["794", "359", "651"],
            seventh: processArrayResults((prizeData.bay || "").split(" ").filter(Boolean)) || ["58", "94", "71", "23"]
          }
        };
        return processedResults;
      } catch (err) {
    ...
    if (minhngocTable.length > 0) {
      ...
      if (results.special || results.first || results.second.length > 0) {
        ...
        const processedResults = {
          ...
          drawTime: "18:15 - 19:15",
          results: {
            special: processNumber(results.special) || "92568",
            first: processNumber(results.first) || "48695",
            second: processArrayResults(results.second) || ["92735", "19304"],
            third: processArrayResults(results.third) || ["39857", "90815", "16359", "83649", "21947", "12376"],
            fourth: processArrayResults(results.fourth) || ["1947", "3658", "7539", "5824"],
            fifth: processArrayResults(results.fifth) || ["5297", "8714", "3852", "2957", "0463", "3175"],
            sixth: processArrayResults(results.sixth) || ["794", "359", "651"],
            seventh: processArrayResults(results.seventh) || ["58", "94", "71", "23"]
          }
        };
        return processedResults;
      }
...
function processNumber(number: string | undefined): string | undefined {
  // Add your number processing logic here.  For example, you might remove non-numeric characters.
  if (number === undefined) return undefined;
  return number.replace(/[^0-9]/g, "");
}

function processArrayResults(numbers: string[]): string[] {
  // Add your array processing logic here.  For example, you might trim whitespace from each number.
  return numbers.map(num => num.trim()).filter(num => num.length > 0);
}